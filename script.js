export function createCluster() {
  ...
  d3.json("data.json").then((data) => {
    ...
    const simulation = d3
      ...
      .force("charge", d3.forceManyBody().strength(-2)) // Soft repulsion to avoid excessive spread
      ...
      .force("collide", d3.forceCollide(7)) // Prevent overlap, ensure separation
    ...
    const bubbles = svg
      ...
      .on("mouseover", (event, d) => {
        tooltip
          ...
          .html(
            `Carbs: ${d.carbs}g <br> Protein: ${d.protein}g <br> Fat: ${d.fat}g <br> Fiber: ${d.fiber}g <br> Insulin: ${d.insulin}`
          );
      })
      .on("mousemove", (event) => {
        tooltip
    ...
    window.addUserData = function () {
      ...
      let insulinLevel =
        carbs * 0.2 + protein * 0.1 + fat * 0.15 - fiber * 0.05;
      ...
      let newBubble = svg
        ...
        .on("mouseover", (event) => {
          tooltip
            .style("visibility", "visible")
            .html(
              `Carbs: ${carbs}g <br> Protein: ${protein}g <br> Fat: ${fat}g <br> Fiber: ${fiber}g <br> Insulin: ${insulinLevel.toFixed(
                2
              )}`
            );
        })
