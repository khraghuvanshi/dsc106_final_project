...
document.addEventListener("DOMContentLoaded", function () {
  ...
  scroller
    .setup({
      step: "#text-container, #chart-container", // Elements to trigger on
      offset: 0.5, // Trigger when 50% of the element is in view
      progress: true, // Enable progress tracking
    })
    .onStepEnter((response) => {
      // Fade out the title when the text container enters
      if (response.element.id === "text-container") {
        titleContainer.style.opacity = 0;
        textContainer.style.opacity = 1;
        textContainer.style.zIndex = 3; // Bring text container to the top
        chartContainer.style.zIndex = 1; // Keep chart container below
      }

      // Fade out the text container when the chart container enters
      if (response.element.id === "chart-container") {
        textContainer.style.opacity = 0;
        chartContainer.style.opacity = 1;
        chartContainer.style.zIndex = 3; // Bring chart container to the top
        textContainer.style.zIndex = 2; // Keep text container below
      }
    })
    .onStepExit((response) => {
      // Fade in the title when the text container exits (only if scrolling up)
      if (
        response.element.id === "text-container" &&
        response.direction === "up"
      ) {
        titleContainer.style.opacity = 1;
        textContainer.style.opacity = 0;
        titleContainer.style.zIndex = 3; // Bring title container to the top
        textContainer.style.zIndex = 2; // Keep text container below
      }

      // Fade in the text container when the chart container exits (only if scrolling up)
      if (
        response.element.id === "chart-container" &&
        response.direction === "up"
      ) {
        textContainer.style.opacity = 1;
    ...
    function updateChart(participant) {
      ...
      dots
        ...
        .on("mouseover", (event, d) => {
          ...
          // Cap "Amount Consumed" at 100
          const amountConsumed = Math.min(+d["Amount Consumed"], 100) / 100;

          // Calculate adjusted values
          const adjustedCalories = (+d.Calories * amountConsumed).toFixed(2);
          const adjustedCarbs = (+d.Carbs * amountConsumed).toFixed(2);
          const adjustedProtein = (+d.Protein * amountConsumed).toFixed(2);
          const adjustedFat = (+d.Fat * amountConsumed).toFixed(2);
          const adjustedFiber = (+d.Fiber * amountConsumed).toFixed(2);
          ...
          tooltip
            ...
            Carbs: ${adjustedCarbs}<br>
            Protein: ${adjustedProtein}<br>
            Fat: ${adjustedFat}<br>
            Fiber: ${adjustedFiber}<br>
        `);
        })
        .on("mouseout", () => {
